<template>
  <div class="monitor-tab">
    ...
    <!-- 热点列表 -->
    <div v-if="loading && records.length === 0" class="list-loading">
      <a-spin />
    </div>
...
const importanceLabel = (v: string) =>
  ({ urgent: '紧急', high: '重要', medium: '中等', low: '一般' }[v] || v)

const importanceColor = (v: string) =>
  ({ urgent: 'red', high: 'volcano', medium: 'gold', low: 'blue' }[v] || 'default')
...
const fmt = (n: number) =>
  n >= 10000 ? `${(n / 10000).toFixed(1)}万` : n >= 1000 ? `${(n / 1000).toFixed(1)}k` : String(n)
